  ...
  <div>
    ...
    <el-card>
      <div align="center">
        ...
        <h2 style="font-weight: bolder;font-size: 20px">钢件生产联络单</h2>
      </div>
  ...
  methods: {
    ...
    handleQuery() {
      ...
      if(this.queryParamsSearch.prodOrderNo!=""&&this.queryParamsSearch.prodOrderNo!=null){
        if (this.queryParamsSearch.prodOrderNo.substring(0,2).toUpperCase() == 'GD'|| this.queryParamsSearch.prodOrderNo.substring(0,2).toUpperCase() == 'BD'){
          this.queryParams.pageNo = 1;
          // console.log(this.queryParamsSearch)
          this.getListSearch();
        }else if(this.queryParamsSearch.prodOrderNo.substring(0,2).toUpperCase() == 'EL'){
    ...
    handleSeeDrawing(row, i) {
      ...
      if (suffix === 'prt' || suffix === 'dwg' || suffix === 'svlx' || suffix === 'vsf') {
        if (suffix === 'prt') {
          ...
          // console.log(url)
          // http://localhost:8803/draw/12745034/WD2304130013/258e9647-9f73-4c16-9c98-a2be25e3310e.dwg.vsf
          const { href } = this.$router.resolve({
