  ...
  <div class="app-container">
        ...
        <el-row>
          ...
          <el-form-item label="工段：" prop="keywords">
            <!-- <el-input
              ...
              @keyup.enter.native="handleQuery"
            /> -->
            <el-select v-model="queryParams.codeList"  filterable placeholder="请选择" clearable>
              <el-option
    ...
    <div>
      <el-table v-loading="loading" :data="sheduleList" border max-height="800px" @selection-change="handleSelectionChange" highlight-current-row>
        ...
        <el-table-column label="电极序号" align="center" prop="suffixs" fixed/>
        ...
        <el-table-column label="工时完成比例" align="center" prop="completionRatio" width="200px">
          <template slot-scope="scope">
            <el-progress :text-inside="true" :stroke-width="20" :percentage="validPercentage(scope.row.completionRatio)" :color="customColorMethod"></el-progress>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    validPercentage(completionRatio) {
      // 确保进度值是一个数字并且在 0 到 100 之间
      const percentage = parseFloat(completionRatio);
      if (isNaN(percentage)) {
        return 0;
      }
