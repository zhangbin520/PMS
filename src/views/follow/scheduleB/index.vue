      ...
      <el-form :model="queryParams" ref="queryForm" :inline="true">
        <el-row>
          ...
          <el-form-item label="部门" prop="keywords">
            <treeselect v-model="queryParams.deptCodeList" :options="deptOptions" :load-options="loadOptions" :normalizer="normalizer" style="width:200px" placeholder="请选择部门">
    ...
    async loadOptions({ action, parentNode, callback }) {
      try {
        ...
        if (action === 'LOAD_ROOT_OPTIONS') {
          response = await fetch('/system/dept/treeselect');
        } else if (action === 'LOAD_CHILDREN_OPTIONS' && parentNode) {
          response = await fetch(`/system/dept/treeselect',/${parentNode.id}`);
        }
