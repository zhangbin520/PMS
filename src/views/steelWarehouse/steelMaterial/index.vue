...
<script>
import {inventoryElectrodeMaterial, incomeElectrodeMaterial,allTexture,listElectrodeMaterial, getElectrodeMaterial, delElectrodeMaterial, addElectrodeMaterial, updateElectrodeMaterial, exportElectrodeMaterial } from "@/api/jy/steelMaterial";
...
export default {
  ...
  methods: {
    ...
    handleIncome(val){
        ...
        this.$prompt(`请输入入库数量\(${val.materialUnit}\)`, `${val.materialCode}`, {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          inputPattern: /[0-9.]/,
          inputErrorMessage: '请输入数字'
          ...
          val.materialWeight = value
          incomeElectrodeMaterial(val).then(resp=>{
            this.$message({
              type: 'success',
              message: '入库成功'
            });
