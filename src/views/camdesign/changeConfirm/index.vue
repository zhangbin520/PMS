<template>
  <div class="app-container">
    <el-row>
      <el-col :span="24">
        <el-form :model="queryParams" ref="queryForm" v-show="showSearch" :inline="true"  label-width="100px">
          ...
          <el-button type="primary" size="small" round style="margin-left: 10px" icon="el-icon-search" @click="handleQuery">查询</el-button>
        </el-form>
    ...
    <el-row>
      <el-col :span="24">
        <el-table ref="multipleTable" v-loading="loading" :data="programmeList" max-height="690px" @selection-change="handleSelectionChange" border highlight-current-row>
          ...
          <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width" fixed>
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" content="绑定电极" placement="top-start">
                <el-button
                  type="primary"
                  circle
                  icon="el-icon-box"
                  @click="openBindPage(scope.row)"
                ></el-button>
              </el-tooltip>
        ...
        <pagination
          ...
          @pagination="getListProgrammeChange"
        />
    ...
    <!-- 新增文件类型对话框 -->
    <el-dialog :title="programNew.title" :visible.sync="programNew.open" width="1200px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="programNew.open = false">取 消</el-button>
      </div>
    </el-dialog>
...
<script>
import {
  listProgramme,listProgrammeChange,
...
export default {
  ...
  created() {
    this.getListProgrammeChange();
    this.getListBrand();
  },
  methods: {
    /** 分页查询变更的编程列表 */
    getListProgrammeChange() {
      
      this.loading = true;
      listProgrammeChange(this.queryParams).then(response => {
...
/** 新增文件类型 */
    openBindPage(row) {
      this.programNew.open = true;
      this.reset();
      this.programNew.title = "新增文件类型";
    },
