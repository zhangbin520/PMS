        ...
        <el-row>
          <el-form-item label="加工任务状态：" prop="taskStatusList">
            <el-checkbox-group v-model="queryParams.taskStatusList">
              <el-checkbox v-for="item in taskStatusOptions" :key="item.status" :label="item.status">{{ item.name }}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
...
import {
  ...
  forceOneButtonWorkReporting, rfidTurnoverBasket, forceOneButtonWorkReportingBySteelParts
  ...
  methods: {
    ...
    getListTask() {
      ...
      listTaskStation(this.queryParams).then(response => {
        ...
        this.loading = false;
        console.log(this.taskStationList)
      })
    },
    /** 查询设备列表 */
    getListEquipment() {
    ...
    handleChangeStatus(row) {
      ...
      this.$modal.confirm('确认要改变任务状态嘛？').then(function () {
        return tubeSetSectionStatus({'taskId':taskId});
      }).then(() => {
    ...
    handleForce(row){
      console.log(row)
      if(row.datasource == '1'){// 电极
        forceOneButtonWorkReporting({'taskId':row.id,'turnoverBasketNoList':[]}).then(res=>{
          this.$modal.msgSuccess("强制报工成功");
          this.getListTask();
        })
      }else if(row.datasource == '0'){// 钢件
        forceOneButtonWorkReportingBySteelParts({'id':row.id}).then(res=>{
          this.$modal.msgSuccess("强制报工成功");
          this.getListTask();
        })
